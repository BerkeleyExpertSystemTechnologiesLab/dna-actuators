...
% Convert the simulation length data from inches to cm:
simulation_lengths_cm = 2.54 * simulation_lengths;
...
plot(experimental_rotations, experimental_lengths, 'b');
...
% For error calculations, first calculate the simulated lengths at the
% same rotations as the experiment.
...
for i = 1: num_datapoints
    ...
    sim_length_current = helix_length(L_cm, r_cm, 2*experimental_rotations(i));
    simulation_lengths_forerrorcalc(i) = sim_length_current;
end

% Note: these errors are for the case where the simulation is performed
% in centimeters, instead of performed in centimeters then converted
% into inches. So, the errors are slightly different than what might be
% expected through other calculations.
